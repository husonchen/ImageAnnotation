...
class DbpediaQuery :
    ...
    def __init__(self):
        ...
        self.con = lite.connect('../python/resource.db')
    ...
    def getConceptsOfThing(self,thing) :
        thing = thing.lower()
        cur = self.con.cursor()
        cur.execute("SELECT concept FROM subject WHERE resource=?" ,[thing])
        rows = cur.fetchall()
        concepts = []
        for row in rows :
    ...
    def findBroaderCategories(self,concepts):
        ...
        sql = "SELECT broader FROM category WHERE concept in (%s)" %placeHolders

        cur.execute(sql,concepts)
...
if __name__ == '__main__':
    ...
    print("SmallestDistance between water and lake " + str(query.getSmallestDistance("Trump_International_Hotel_Las_Vegas","Donald_Trump")))
    end = time.time() * 1000
